...
type TokenWorker interface {
	GenerateToken(userClaims UserClaims, exp int64, secretKey string) (string, error)
...
func (w *tokenWorker) GenerateToken(userClaims UserClaims, exp int64, secretKey string) (string, error) {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"user_id":   userClaims.GetID(),
		"email":     userClaims.GetEmail(),
		"user_name": userClaims.GetUsername(),
		"is_admin":  userClaims.GetIsAdmin(),
		"exp":       exp,
	})
